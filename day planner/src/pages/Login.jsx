...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('https://moodmint-ozqw.onrender.com/api/auth/login', formData);
    ...
    <div className="min-h-screen bg-gradient-pastel flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-gradient-to-r from-pastel-darkPink to-pastel-darkLavender text-white font-bold rounded-xl py-3 shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all mt-6">
            Log In
          </button>
        </form>

        <p className="text-center text-gray-600 mt-6">
          Don't have an account? <Link to="/signup" className="text-pastel-darkPink font-semibold hover:underline">Sign up</Link>
        </p>
      </motion.div>
      
      {/* Background Blobs */}
      <div className="fixed top-20 right-10 w-64 h-64 bg-pastel-darkLavender rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
      <div className="fixed bottom-20 left-10 w-72 h-72 bg-pastel-pink rounded-full mix-blend-multiply filter blur-3xl opacity-30"></div>
    </div>
  );
};
